...
(defn create-chart
  ...
  [table-element target-element chart-type stacked?]
...
(defn draw
  ...
  (doseq [table (d/sel [:.chart.bar.stacked])]
	(create-chart table (d/insert-before! (d/create-element :div) table) :bar-chart true)
	(d/remove-class! table "chart") ; so table isn't converted to chart again
	(d/hide! table))

  (doseq [table (d/sel [:.chart.column])]
	(create-chart table (d/insert-before! (d/create-element :div) table) :column-chart false)
